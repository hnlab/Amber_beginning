...
for system in ligands; do
  ...
  for w in 0.00922 0.04794 0.11505 0.20634 0.31608 0.43738 0.56262 0.68392 0.79366 0.88495 0.95206 0.99078; do
      ...
      ln -sf ${parmfiles}/${system}_vdw_bonded.rst7  ti.rst7
      cp /pubhome/yjcheng02/kinase/A397_sel/MD_tools/ABFE_restrain/qsub_complex.sh .
      sed -i "s/mmpbsa/abfe_lig/g" qsub_complex.sh
      # sed -i "s/ampere/cuda/g" qsub_complex.sh
      # sbatch qsub_complex.sh
      cd ..
  done
  cd $top
done
...
for system in recomplex; do
  ...
  for w in 0.00922 0.04794 0.11505 0.20634 0.31608 0.43738 0.56262 0.68392 0.79366 0.88495 0.95206 0.99078; do
      ...
      sed -e "s/%L%/$w/" -e "s/%FE%/$vdw_bonded_re/" /pubhome/yjcheng02/kinase/A397_sel/MD_tools/ABFE_restrain/min.tmpl > $w/min.in
      sed -e "s/%L%/$w/" -e "s/%FE%/$vdw_bonded_re/" /pubhome/yjcheng02/kinase/A397_sel/MD_tools/ABFE_restrain/heat.tmpl > $w/heat.in
      sed -e "s/%L%/$w/" -e "s/%FE%/$vdw_bonded_re/" /pubhome/yjcheng02/kinase/A397_sel/MD_tools/ABFE_restrain/prod.tmpl > $w/ti.in
      sed -i "s/noshakemask = ':1,2'/noshakemask = ':1'/g" $w/*.in
      sed -i "s/timask1 = ':1'/timask1 = ''/g" $w/*.in
      sed -i "s/timask2 = ':2'/timask2 = ':1'/g" $w/*.in
      sed -i "s/6000000/18000000/g" $w/ti.in

      cd $w
      ln -sf ${parmfiles}/complex_vdw_bonded.parm7 ti.parm7
      ln -sf ${parmfiles}/complex_vdw_bonded.rst7  ti.rst7
      cp /pubhome/yjcheng02/kinase/A397_sel/MD_tools/ABFE_restrain/qsub_complex.sh .
      # sed -i "s/ampere/cuda/g" qsub_complex.sh
      # qsub qsub_complex.sh
      # sbatch qsub_complex.sh
      cd ..
  done
  cd $top
